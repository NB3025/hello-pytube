...
def seq_stream(
    ...
    base_url = '%s://%s/%s?' % (split_url.schme, split_url.netloc, split_url.path)
    ...
    while seq_num <= segment_count:
        ...
        yield from stream(url, timeout=timeout, max_retries=max_retries) #TODO yield 뒤에 from으로 .. 이런 문법은 뭐지
        seq_num +=1
...
def stream(
    ...
    while downloaded < file_size:
        ...
        while True:
            ...
            yield chunk #TODO chunk 개념이 뭐지
    return # pylint: disable=R1711 #TODO pylint가 뭐지


@lru_cache() #TODO lru_cache가 뭐지
def filesize(url):
